...
function App() {
  ...
  const Modal = ({open, setOpen, children}) => (
    <div className={`overlay animated ${open ? 'show' : ''}`}>
          <div className='modal'>
            {children}
            <svg onClick={() => setOpen(false)} className='cross-button' width="30px" height="30px" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div className="App">
      ...
      <div className='modal-container'>
        ...
        <Modal open={open} setOpen={setOpen}>
          <img className='image' src="./bugatti.jpg" width={300} alt="Bugatti" />
        </Modal>
      </div>
    </div>
  );
